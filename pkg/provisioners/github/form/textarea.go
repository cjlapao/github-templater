...
func (i TextAreaItem) Render(v string) {
	i.Attributes["render"] = v
}
...
func (i TextAreaItem) Validate() diagnostics.Diagnostics {
	...
	if label, ok := i.Attributes["label"].(string); ok {
		if label == "" {
			diag.AddError(fmt.Errorf("textarea %v label is required", i.ID))
		}
	} else {
		if i.Attributes["label"] == nil {
			diag.AddError(fmt.Errorf("textarea %v label is required", i.ID))
		} else {
			diag.AddError(fmt.Errorf("textarea %v label is not a string", i.ID))
		}
	}

	languageFound := false
	for _, language := range renderLanguages {
		if _, ok := i.Attributes["render"]; ok {
			if language == i.Attributes["render"] {
				languageFound = true
				break
			}
		}
	...
	if _, ok := i.Attributes["render"]; ok {
		currentLanguage := i.Attributes["render"].(string)
		if !languageFound {
			diag.AddError(fmt.Errorf("textarea %v render language %v is not valid", i.ID, currentLanguage))
		}
	}
